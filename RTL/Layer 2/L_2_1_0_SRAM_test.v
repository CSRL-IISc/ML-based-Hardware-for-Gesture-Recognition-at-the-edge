...
module L_2_1_0_SRAM_test #(
    ...
    output reg  [N-1:0] read_data_02,
    output reg  [N-1:0] read_data_03,
    output reg  [N-1:0] read_data_04,
    ...
    reg [N-1:0] rom2 [0:MAX_ADDR];
    reg [N-1:0] rom3 [0:MAX_ADDR];
    reg [N-1:0] rom4 [0:MAX_ADDR];

    reg [N-1:0] rom1_q, rom2_q, rom3_q, rom4_q;
    ...
    always @(posedge clk) begin
        if (preload_en) begin
            ...
            rom2[preload_addr] <= preload_data2;
            rom3[preload_addr] <= preload_data3;
            rom4[preload_addr] <= preload_data4;
        end
    end
    ...
    always @(negedge clk) begin
        rom1_q <= rom1[wr_addr_q];
        rom2_q <= rom2[wr_addr_q];
        rom3_q <= rom3[wr_addr_q];
        rom4_q <= rom4[wr_addr_q];
    end
    ...
    // ============================================================
    reg [M-1:0] aa_d;
    reg [N-1:0] da1_d, da2_d, da3_d, da4_d;
    reg         we_d, ce_d;
    reg [N-1:0] bwe_d;
    ...
    always @(*) begin
        case(state)
            ...
            WRITE, WRITE_LAST: begin
                ...
                da2_d = rom2[wr_addr_q];
                da3_d = rom3[wr_addr_q];
                da4_d = rom4[wr_addr_q];
    ...
    // ============================================================
    TSDN65LPLLA1024X8M8M S1 ( .AA(aa_1), .DA(da_1), .BWEBA(bwe_1), .WEBA(we_1), .CEBA(ce_1),
        .CLKA(clk), .QA(qa_1), .AB(0), .DB(0), .BWEBB({N{1'b1}}),
        .WEBB(1'b1), .CEBB(1'b1), .CLKB(1'b0), .AMA(0), .DMA(0),
        .BWEBMA({N{1'b1}}), .WEBMA(1'b1), .CEBMA(1'b1),
        .AMB(0), .DMB(0), .BWEBMB({N{1'b1}}), .WEBMB(1'b1),
        .CEBMB(1'b1), .BIST(1'b0), .AWT(1'b0), .CLKM(1'b0)
    );

    TSDN65LPLLA1024X8M8M S2 ( .AA(aa_2), .DA(da_2), .BWEBA(bwe_2), .WEBA(we_2), .CEBA(ce_2),
        .CLKA(clk), .QA(qa_2), .AB(0), .DB(0), .BWEBB({N{1'b1}}),
        .WEBB(1'b1), .CEBB(1'b1), .CLKB(1'b0), .AMA(0), .DMA(0),
        .BWEBMA({N{1'b1}}), .WEBMA(1'b1), .CEBMA(1'b1),
        .AMB(0), .DMB(0), .BWEBMB({N{1'b1}}), .WEBMB(1'b1),
        .CEBMB(1'b1), .BIST(1'b0), .AWT(1'b0), .CLKM(1'b0)
    ...
    TSDN65LPLLA1024X8M8M S3 ( .AA(aa_3), .DA(da_3), .BWEBA(bwe_3), .WEBA(we_3), .CEBA(ce_3),
        .CLKA(clk), .QA(qa_3), .AB(0), .DB(0), .BWEBB({N{1'b1}}),
        .WEBB(1'b1), .CEBB(1'b1), .CLKB(1'b0), .AMA(0), .DMA(0),
        .BWEBMA({N{1'b1}}), .WEBMA(1'b1), .CEBMA(1'b1),
        .AMB(0), .DMB(0), .BWEBMB({N{1'b1}}), .WEBMB(1'b1),
        .CEBMB(1'b1), .BIST(1'b0), .AWT(1'b0), .CLKM(1'b0)
    ...
    TSDN65LPLLA1024X8M8M S4 ( .AA(aa_4), .DA(da_4), .BWEBA(bwe_4), .WEBA(we_4), .CEBA(ce_4),
        .CLKA(clk), .QA(qa_4), .AB(0), .DB(0), .BWEBB({N{1'b1}}),
        .WEBB(1'b1), .CEBB(1'b1), .CLKB(1'b0), .AMA(0), .DMA(0),
        .BWEBMA({N{1'b1}}), .WEBMA(1'b1), .CEBMA(1'b1),
        .AMB(0), .DMB(0), .BWEBMB({N{1'b1}}), .WEBMB(1'b1),
        .CEBMB(1'b1), .BIST(1'b0), .AWT(1'b0), .CLKM(1'b0)
